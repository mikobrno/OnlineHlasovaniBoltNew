...
export const PublicVotingPage: React.FC = () => {
  ...
  // GraphQL query for vote details
  const { data: voteData, loading: voteLoading, error: voteError } = useQuery(GET_VOTE_DETAILS, {
    variables: { voteId: votingToken?.voteId },
    skip: !votingToken?.voteId,
  });
  ...
  // Extracted vote data from GraphQL
  const vote = voteData?.vote || null;
  
  // TODO: Replace with GraphQL queries for member and building data
  const member = null; // votingToken ? members.find(m => m.id === votingToken.memberId) : null;
  const building = null; // vote ? buildings.find(b => b.id === vote.buildingId) : null;
  ...
  const handleSubmitVote = async () => {
    ...
    const allAnswered = vote.questions.every((q: any) => answers[q.id]);
    if (!allAnswered) {
